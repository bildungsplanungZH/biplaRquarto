...
.First <- function() {
  ...
  options(defaultPackages = unique(c(
...
.get_profile <- function() {
  ...
  f_check <- function(.x) {
    ...
    if (!all(c("r_user", "r_settings") %in% names(.x))) {
      return(FALSE)
    }
    return(TRUE)
  }
  ...
  if (f_check(profile_user)) {
    profile$r_user <- profile_user
  } else {
    profile$r_user <- NULL
  }
  ...
  if (f_check(defpacks)) {
    profile$r_settings$defpacks <- defpacks
  } else {
    profile$r_settings$defpacks <- NULL
  }
...
.read_profile_yml <- function(yml_file) {
  if (file.exists(yml_file)) {
    ...
    f_get_idx_sec <- function(.x) {
      max(idx_sec[which(.x >= idx_sec)])
    }
    ...
    f_tbl <- function(.tbl) {
      stats::setNames(lapply(as.list(.tbl$value), f_lgl), .tbl$var)
    }
