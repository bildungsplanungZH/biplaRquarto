...
use_quarto <- function(file_name = "report", ext_name = "biplaR-html",
  ...
  readLines(paste0(file_name, ".qmd", collapse = "")) |>
    gsub(pattern = "Vorname Nachname",
         replacement = paste(author_info$user$given,
                             author_info$user$family), x = _) |>
    gsub(pattern = "vorname.nachname@bi.zh.ch",
         replacement = author_info$user$email, x = _) |>
    gsub(pattern = "Organisationsname",
         replacement = paste0(author_info$org$org1), x = _) |>
    gsub(pattern = "Direktion",
