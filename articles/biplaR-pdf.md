...
Die Vorlage biplaR-pdf funktioniert mit MikTeX. Mit TinyTeX wurde sie
nicht ausführlich getestet. MikTeX kann über das AFI-ServicePortal
bestellt werden.

Nach der Installation muss der Pfad zum Verzeichnis `biplaR-pdf` in
MikTeX zu den TEXMF-Wurzelverzeichnissen hinzugefügt werden:  
*Einstellungen -\> Verzeichnisse -\> hinzufügen via Plus-Zeichen*

Der Warnhinweis (kein TDS-konformes Wurzelverzeichnis) kann ignoriert
werden
...
- *MiKTeX Console*, **Aufgaben**, **Dateinamendatenbank aktualisieren**
  ...
  - bei Änderungen innerhalb der aufgeführten Directories (va. bei neuen
    Files)
  - bei allen Fehlermeldungen mit `not found`
- *MiKTeX Console*, **Pakete**
  - zur Installation neuer Packages
- *MiKTeX Console*, **Aufgaben**, **Nach Updates suchen** und **Jetzt
  updaten**
  - bei Fehlermeldungen aus einzelnen Packages
- *MiKTeX Console*, **Pakete**, Package **dehyph-exptl** installieren,
  anschliessend **Aufgaben**, **Dateinamendatenbank aktualisieren**
  - behebt Fehler `Unknown pattern 'ngerman-x-latest'.`, verursacht
    durch `\RequirePackage[ngerman=ngerman-x-latest]{hyphsubst}` in
    Dokumentenklasse `biplaRartcl`
